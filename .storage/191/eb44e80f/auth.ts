...
class AuthService {
  ...
  private async initializeFromToken() {
    ...
    if (token) {
      try {
        ...
        console.log('✅ User authenticated from stored token:', this.currentUser.email);
      } catch (error) {
        console.log('❌ Invalid stored token, clearing authentication');
        this.logout();
      }
    }
  }
